...
function Reservar_parqueadero(props){
    ...
    const [parqeuaderoSel , setParqueaderoSel] = useState('');
    ...
    const goBack = () => { RootNavigation.navigate('Home_parqueadero') }


    async function pedirPermisoNotificaciones() {
    if (Platform.OS === 'android') {
        console.log('solicitando permisos SP')
        const granted = await PermissionsAndroid.request(
        PermissionsAndroid.PERMISSIONS.POST_NOTIFICATIONS,
        {
            title: 'Permiso de Notificaciones',
            message: 'Necesitamos mostrar recordatorios de tu reserva.',
    ...
    const iniciarSeguimientoReserva = async () => {
        console.log('iniciar reserva en seg plano')
        await pedirPermisoNotificaciones();
        NotificacionModule.iniciarNotificaciones('reserva');
    };
    ...
    const guardar_reserva = async () => {
        ...
        await guardarRecordatorioStorage();
    ...
    const lugarSeleccionado = async(data) => {
        ...
        if (data.estado === 'DISPONIBLE') {
            setState({ ...state, ticket: data.id, numLugar: data.numero, qr: data.qr })
        }else {
            setModalError(true);
        }
    }
    ...
    const viewLugar = async (data) => {
        ...
        if (data.duracion_reserva_min === 0) {
            Alert.alert("Mensaje", "⚠️ Este parqueadero no tiene reservas activas");
            return
        }
        setParqueaderoSel(data.id);
    ...
    useEffect(() => {
        if (props.parqueaderosData.reservaSave) {
            RootNavigation.navigate('Home_parqueadero')
        }
    ...
    useEffect(() => {
        ...
        //iniciarSeguimientoReserva('1234');
    ...
    <SafeAreaView style={estilos.safeArea}>
        ...
        <View style={styles.cajaCabeza}>
          ...
          <Text style={estilos.title}>Reserva tu parqueadero</Text>
        </View>    
        ...
        <View>
            <View style={estilos.contenedor}>
                {
                    ...
                    <>
                        ...
                        <RNPickerSelect
                            ...
                            placeholder={{ label: 'Selecciona un parqueadero', value: '' }}
                            ...
                            //se va a filtrar por estado del parqueadero para que solo muestro los parqueaderos
                            //acá se filtra por PARQUEADERO
                            items={props.parqueaderosData.parqueaderosx.data
                                .filter(valor => valor.estado === 'PARQUEADERO')
                                .map((data) =>
            ...
            
            {
                ...
                <View style={estilos.cajaBtnReservar}>
                    <Text style={estilos.titleSelect4}>Parqueadero Seleccionado</Text>
...
export default connect(mapStateToProps)(Reservar_parqueadero);