...
function CarpooolingTYCDriver (props) {
    ...
    const [ daviplata, setDaviplata ] = useState(true);
    const [ nequi, setNequi ] = useState(true);
      ...
      const aceptTyC = async () => {
        ...
        if (props.perfil.dataempresa[0]._carro_compartido === 'ACTIVO+PAGOS') {
          if (!numero || numero.length !== 10) {
            Alert.alert('Escribe un número válido. (10 digitos)');
            return;
          }
        }
        ...
        let dataJson = {
            ...
            "nombre": nombre === '' ? 'sin datos' : nombre,
            "daviplata": props.perfil.dataempresa[0]._carro_compartido === 'ACTIVO+PAGOS' ? daviplata : !daviplata,
            "nequi": props.perfil.dataempresa[0]._carro_compartido === 'ACTIVO+PAGOS' ? nequi : !nequi,
        };
        try {
            await dispatch(accept_tyc('Conductor', dataJson));
            props.closeModal();
        } catch (error) {
          ...
          <View style={styles.cajaTyC}>
            ...
            <View  style={styles.cajaAceptarContinuar}>
              ...
              <View style={{ 
                ...
                <Text style={{ fontSize: 16, fontFamily: Fonts.$poppinsregular }}>Información Daviplata y Nequi</Text>
                <TextInput
                  style={styles.input}
                ...
                <TextInput
                  ...
                  placeholder="Nombre"
                />
                <Text style={{ fontSize: 16, fontFamily: Fonts.$poppinsregular }}>¿Que medio de pago recibes?</Text>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 10 }}>
                  <Text style={{width: 100, fontSize:18, marginTop:2, fontFamily: Fonts.$poppinsregular}}>Daviplata</Text>
                  <Pressable
                    onPress={() => setDaviplata(!daviplata)}
                    style={ daviplata ? styles.btnCheckOK : styles.btnCheck }
                  />
                  <Text style={{width: 100, fontSize:18, marginTop:2, fontFamily: Fonts.$poppinsregular}}>Nequi</Text>
                  <Pressable
                    onPress={() => setNequi(!nequi)}
                    style={ nequi ? styles.btnCheckOK : styles.btnCheck }
                  />
                </View>
              </View>:null}
              
              <View style={[styles.cajaScrool, {
                height: props.perfil.dataempresa[0]._carro_compartido === 'ACTIVO+PAGOS' ? "30%" : "70%"
              }]}>
                <TYC />
              </View>
