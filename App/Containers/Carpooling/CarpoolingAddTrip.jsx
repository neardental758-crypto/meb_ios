...
function CarpoolingAddTrip(props) {
  ...
  useEffect(() => {
    ...
    if (btnpuntoPC === true) {
      ...
      setSearchText(directionNameUser); // AGREGADO: Actualiza el valor del TextInput
      setPosition1({ lat: directionUser.latitude, lng: directionUser.longitude });
    } else {
      setSearchQuery('');
      setSearchText(''); // AGREGADO: Borra el valor del TextInput
      setPosition1(null);
    }
  }, [btnpuntoPC, props.dataCarpooling.directionUser, props.dataCarpooling.directionNameUser]); // AGREGADO: Dependencias para evitar warnings y asegurar que se actualice
  ...
  useEffect(() => {
    ...
    if (btnpuntoPO === true) {
      ...
      setSearchTextDestiny(directionNameUser); // AGREGADO: Actualiza el valor del TextInput
      setPosition2({ lat: directionUser.latitude, lng: directionUser.longitude });
    } else {
      setSearchQueryDestiny('');
      setSearchTextDestiny(''); // AGREGADO: Borra el valor del TextInput
      setPosition2(null);
    }
  }, [btnpuntoPO, props.dataCarpooling.directionUser, props.dataCarpooling.directionNameUser]); // AGREGADO: Dependencias para evitar warnings y asegurar que se actualice
  ...
  useEffect(() => {
    ...
    const direccionLista = props.dataCarpooling.directionUser;

    if (ubicacionLista && vehiculosListos && direccionLista) {
      const delay = setTimeout(() => setLoadingScreen(false), 800); // pequeño delay visual
    ...
    props.dataCarpooling.directionUser
  ]);
